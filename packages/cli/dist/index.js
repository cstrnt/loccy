...
var require_dist = __commonJS({
  "../shared/dist/index.js"(exports, module2) {
    ...
    __export(src_exports, {
      DEFAULT_FILE_NAME: () => DEFAULT_FILE_NAME2,
      asConfig: () => asConfig,
      localeFileSchema: () => localeFileSchema2
    });
    module2.exports = __toCommonJS(src_exports);
    var import_zod = require_lib();
    var localeFileSchema2 = import_zod.z.object({
    ...
    var DEFAULT_FILE_NAME2 = "loccy.config.mjs";
  }
});
...
console.log(import_chalk.default.red(import_figlet.default.textSync("loccy", { horizontalLayout: "full" })));
import_commander.program.name("loccy").description("The offical loccy cli").version("0.0.1");
import_commander.program.command("sync").description("Sync your current loccy config").option("-f, --file <string>", "path to loccy config file").option("-t, --token <string>", "your loccy auth token").action(async (options) => {
  console.log(options.file, options.token);
  try {
    const data = (await import(import_path.default.join(process.cwd(), options.file || import_shared.DEFAULT_FILE_NAME))).default;
    const config = await import_shared.localeFileSchema.parseAsync(data);
    const { status } = await (0, import_undici.fetch)("http://localhost:3000/api/config/push", {
      method: "POST",
    ...
    console.error(e);
    console.log(import_chalk.default.red("Invalid Config File"));
  }
});
